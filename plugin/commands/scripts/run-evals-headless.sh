...
# phases of /claude-health-check (the ones the deterministic suite
...
#      the command, its scripts and references (so /claude-health-check
...
for f in "$EVALS"/*.json; do
    ...
    for ((r=1; r<=runs; r++)); do
        ...
        # ~/.claude/claude-health-check/references/ fallback path.
        cp "$REFS"/*.md "$tmp/.claude/claude-health-check/references/" 2>/dev/null
        # Reuse the real guidance cache (thresholds) if present so Phase 1 skips 5 WebFetches.
        cp "$HOME/.claude/.cache/claude-health-check-guidance.json" "$tmp/.claude/.cache/" 2>/dev/null || true
