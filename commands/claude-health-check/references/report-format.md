...
- **Skills** — `MISSING-DESC` `WEAK-DESC` `UNDER-TRIGGER` `OVER-TRIGGER` `NEEDS-REFERENCES` `BURIED-CRITICAL` `BAD-NAME` `NAME-MISMATCH` `BAD-FRONTMATTER-SCHEMA` `MODEL-NOT-AVAILABLE` `UNKNOWN-FRONTMATTER-FIELD` `THIRD-PERSON` `DESCRIPTION-TOO-LONG` `SKILL-BUDGET-OVERFLOW` `SKILL-LOW-RELEVANCE` `SKILL-DUPLICATE-DOMAIN` `SKILL-NEVER-FIRED` `SKILL-DORMANT` `SKILL-MISFIRING` `SKILL-ORPHAN` `SKILL-TOOL-UNUSED` `SKILL-TOOL-UNDECLARED` `NAME-COLLISION` `EMBEDDED-SECRET` `UNFLAGGED-DESTRUCTIVE` `BODY-FILLER-HIGH` `BODY-COMPRESSED` `BODY-COMPRESSION-REJECTED` `OVER-500-LINES` `NO-PROGRESSIVE-DISCLOSURE` `DESCRIPTION-TOO-LONG` `DESCRIPTION-TRUNCATED` + a `DEAD-REF` whose path is a skill's own `references/`
...
- **Plugins** — `PLUGIN-BROKEN-REF` `PLUGIN-MISSING-MANIFEST` `PLUGIN-VERSION-DRIFT` `MCP-DEPRECATED-TRANSPORT` `MCP-BAD-DEF` `MCP-PLAINTEXT-SECRET` `PLUGIN-MISPLACED-DIR` `PLUGIN-BAD-VERSION` `PLUGIN-ABS-PATH` `MARKETPLACE-DEAD-SOURCE`
- **Output Styles** — `OUTPUTSTYLE-MISSING`
- **CLAUDE.md** — `CLAUDEMD-STALE` `CLAUDEMD-GENERIC` `CLAUDEMD-THIN` `CLAUDEMD-DEAD-IMPORT` `CLAUDEMD-DEAD-SCRIPT` `IMPORT-TOO-DEEP` `LOCAL-MD-TRACKED` `CLAUDEMD-OBVIOUS` `CLAUDEMD-MEMORY-DRIFT`
- **Rules** — `BAD-RULE-FRONTMATTER` `RULE-OVERSIZED`
- **Context** — `LOW-CACHE-HIT` `CONTEXT-BLOAT` `OVER-CONSTRAINED` `INSTRUCTION-DUPLICATED` `RULE-CONFLICT`
...
```
## CLAUDE.md
score: 72/100 (B) — Commands 9/15 · Architecture 15/15 · Patterns 20/30 · Conciseness 15/15 · Currency 3/15 · Actionability 10/10
...
## Skills
 1. 🔴 must-fix atlassian links to a missing file (skills/atlassian/references/api.md)
               Evidence: → skills/atlassian/references/api.md (missing) — skills/atlassian/SKILL.md   · DEAD-REF
